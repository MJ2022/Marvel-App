package com.example.marvelapp

data class ConstantData(
    val name: String,
    val description: String,
    val img: String
)


class Constant {
    companion object {
        fun getPersonajes(): List<ConstantData> {
            return List(20) {
                ConstantData(
                    "3-D Man",
                    "Description not found",
                    "https://i.annihil.us/u/prod/marvel/i/mg/c/e0/535fecbbb9784.jpg"
                )
                ConstantData(
                    "A-Bomb (HAS)",
                    "Rick Jones has been Hulk's best bud since day one, but now he's more than a friend...he's a teammate! Transformed by a Gamma energy explosion, A-Bomb's thick, armored skin is just as strong and powerful as it is blue. And when he curls into action, he uses it like a giant bowling ball of destruction! ",
                    "https://i.annihil.us/u/prod/marvel/i/mg/3/20/5232158de5b16.jpg"
                )
                ConstantData(
                    "A.I.M.",
                    "AIM is a terrorist organization bent on destroying the world.",
                    "https://i.annihil.us/u/prod/marvel/i/mg/6/20/52602f21f29ec.jpg"
                )
                ConstantData(
                    "Aaron Stack",
                    "Description Not Found",
                    "https://i.annihil.us/u/prod/marvel/i/mg/b/40/image_not_available.jpg"
                )

                ConstantData(
                    "Abomination (Emil Blonsky)",
                    "Formerly known as Emil Blonsky, a spy of Soviet Yugoslavian origin working for the KGB, the Abomination gained his powers after receiving a dose of gamma radiation similar to that which transformed Bruce Banner into the incredible Hulk.",
                    "https://i.annihil.us/u/prod/marvel/i/mg/9/50/4ce18691cbf04.jpg"
                )

                ConstantData(
                    "Abomination (Ultimate)",
                    "Description not found",
                    "https://i.annihil.us/u/prod/marvel/i/mg/b/40/image_not_available.jpg"
                )

                ConstantData(
                    "Absorbing Man",
                    "Description not found",
                    "https://i.annihil.us/u/prod/marvel/i/mg/1/b0/5269678709fb7.jpg"
                )

                ConstantData(
                    "Abyss",
                    "Description not found",
                    "https://i.annihil.us/u/prod/marvel/i/mg/9/30/535feab462a64.jpg"
                )

                ConstantData(
                    "Abyss (Age of Apocalypse)",
                    "Description not found",
                    "https://i.annihil.us/u/prod/marvel/i/mg/3/80/4c00358ec7548.jpg"
                )

                ConstantData(
                    "Adam Destine",
                    "Description not found",
                    "https://i.annihil.us/u/prod/marvel/i/mg/b/40/image_not_available.jpg"
                )

                ConstantData(
                    "Adam Warlock",
                    "Adam Warlock is an artificially created human who was born in a cocoon at a scientific complex called The Beehive.",
                    "https://i.annihil.us/u/prod/marvel/i/mg/a/f0/5202887448860.jpg"
                )

                ConstantData(
                    "Aegis (Trey Rollins)",
                    "Description not found",
                    "https://i.annihil.us/u/prod/marvel/i/mg/5/e0/4c0035c9c425d.gif"
                )

                ConstantData(
                    "Aero (Aero)",
                    "Description not found",
                    "https://i.annihil.us/u/prod/marvel/i/mg/b/40/image_not_available.jpg"
                )

                ConstantData(
                    "Agatha Harkness",
                    "Description not found",
                    "https://i.annihil.us/u/prod/marvel/i/mg/c/a0/4ce5a9bf70e19.jpg"
                )

                ConstantData(
                    "Agent Brand",
                    "Description not found",
                    "https://i.annihil.us/u/prod/marvel/i/mg/4/60/52695285d6e7e.jpg"
                )

                ConstantData(

                    "Agent X (Nijo)",
                    "Originally a partner of the mind-altering assassin Black Swan, Nijo spied on Deadpool as part of the Swan's plan to exact revenge for Deadpool falsely taking credit for the Swan's assassination of the Four Winds crime family, which included Nijo's brother.",
                    "http://i.annihil.us/u/prod/marvel/i/mg/b/40/image_not_available.jpg"
                )

                ConstantData(
                    "Agent Zero",
                    "Description not found",
                    "https://i.annihil.us/u/prod/marvel/i/mg/f/60/4c0042121d790.jpg",
                )
                ConstantData(
                    "Agents of Atlas",
                    "Description not found",
                    "https://i.annihil.us/u/prod/marvel/i/mg/9/a0/4ce18a834b7f5.jpg"
                )
                ConstantData(
                    "Aginar",
                    "Description not found",
                    "https://i.annihil.us/u/prod/marvel/i/mg/b/40/image_not_available.jpg"
                )
                ConstantData(
                    "Air-Walker (Gabriel Lan)",
                    "Description not found",
                    "https://playcontestofchampions.com/wp-content/uploads/2021/11/champion-air-walker.jpg"
                )
            }
        }

        fun getComics(): List<ConstantData> {
            return List(20) {
                ConstantData(
                    " Fantastic Four by Dan Slott Vol. 1 (Hardcover)",
                    "Collects Fantastic Four (2018) #1-11, Fantastic Four Wedding Special (2018) #1. The Fantastic Four is the World’s Greatest Comics Magazine — and superstar writer Dan Slott is about to show you why! After being separated across dimensions, the Thing, the Human Torch, the Invisible Woman and Mister Fantastic reunite at long last! But as Reed and Sue return home with their children, they find that big things have changed! Ben Grimm and Alicia Masters have major plans of their own — like a wedding, years in the making! And a new fabulous foursome, the Fantastix, are muscling in on the scene! But now that the FF are back, can Doctor Doom be far behind? And what about Galactus? Brace yourself for the World-Eater vs. the Lord of Latveria, with the fate of the planet hanging in the balance!",
                    "https://i.annihil.us/u/prod/marvel/i/mg/8/a0/603d5b82a5bc0.jpg"
                )
                ConstantData(
                    " Superior Spider-Man Vol. 2: Otto-matic (Trade Paperback)",
                    "",
                    "https://i.annihil.us/u/prod/marvel/i/mg/c/f0/5df3fc8b3c883.jpg"
                )
                ConstantData(
                    "100th Anniversary Special (2014) #1",
                    "CELEBRATE 100 YEARS OF EARTH&#39;S MIGHTIEST HEROES &ndash; THE AVENGERS! Following the failed Badoon invasion of Earth and America&#39;s disappearance into the Negative Zone, how will the Avengers of 2061 cope?!",
                    "https://i.annihil.us/u/prod/marvel/i/mg/3/a0/53c406e09649c.jpg"
                )
                ConstantData(
                    "100th Anniversary Special (2014) #1",
                    "It&#39;s 100 years after the wall-crawler&#39;s creation, but when the Kingpin has taken Spider-Man&#39;s ultra-powerful techno-symbiote suit, Spider-Man will need to prove once again why he is the world&#39;s greatest super hero.",
                    "https://i.annihil.us/u/prod/marvel/i/mg/c/20/53b1be72eec81-jpg"
                )
                ConstantData(
                    "100th Anniversary Special (2014) #1",
                    "A REMARKABLE ARTIFACT FROM THE FUTURE OF MARVEL COMICS! It's 2061 and the world of the Fantastic Four has turned upside-down, complete with the granddaughter of Doom...and the Richards-Banner twins?",
                    "https://i.annihil.us/u/prod/marvel/i/mg/3/20/53a85058a61f0.jpg"
                )
                ConstantData(
                    " Fantastic Four by Dan Slott Vol. 1 (Hardcover)",
                    "Collects Fantastic Four (2018) #1-11, Fantastic Four Wedding Special (2018) #1. The Fantastic Four is the World’s Greatest Comics Magazine — and superstar writer Dan Slott is about to show you why! After being separated across dimensions, the Thing, the Human Torch, the Invisible Woman and Mister Fantastic reunite at long last! But as Reed and Sue return home with their children, they find that big things have changed! Ben Grimm and Alicia Masters have major plans of their own — like a wedding, years in the making! And a new fabulous foursome, the Fantastix, are muscling in on the scene! But now that the FF are back, can Doctor Doom be far behind? And what about Galactus? Brace yourself for the World-Eater vs. the Lord of Latveria, with the fate of the planet hanging in the balance!",
                    "https://i.annihil.us/u/prod/marvel/i/mg/8/a0/603d5b82a5bc0.jpg"
                )
                ConstantData(
                    " Superior Spider-Man Vol. 2: Otto-matic (Trade Paperback)",
                    "",
                    "https://i.annihil.us/u/prod/marvel/i/mg/c/f0/5df3fc8b3c883.jpg"
                )
                ConstantData(
                    "100th Anniversary Special (2014) #1",
                    "CELEBRATE 100 YEARS OF EARTH&#39;S MIGHTIEST HEROES &ndash; THE AVENGERS! Following the failed Badoon invasion of Earth and America&#39;s disappearance into the Negative Zone, how will the Avengers of 2061 cope?!",
                    "https://i.annihil.us/u/prod/marvel/i/mg/3/a0/53c406e09649c.jpg"
                )
                ConstantData(
                    "100th Anniversary Special (2014) #1",
                    "It&#39;s 100 years after the wall-crawler&#39;s creation, but when the Kingpin has taken Spider-Man&#39;s ultra-powerful techno-symbiote suit, Spider-Man will need to prove once again why he is the world&#39;s greatest super hero.",
                    "https://i.annihil.us/u/prod/marvel/i/mg/c/20/53b1be72eec81-jpg"
                )
                ConstantData(
                    "100th Anniversary Special (2014) #1",
                    "A REMARKABLE ARTIFACT FROM THE FUTURE OF MARVEL COMICS! It's 2061 and the world of the Fantastic Four has turned upside-down, complete with the granddaughter of Doom...and the Richards-Banner twins?",
                    "https://i.annihil.us/u/prod/marvel/i/mg/3/20/53a85058a61f0.jpg"
                )
                ConstantData(
                    " Fantastic Four by Dan Slott Vol. 1 (Hardcover)",
                    "Collects Fantastic Four (2018) #1-11, Fantastic Four Wedding Special (2018) #1. The Fantastic Four is the World’s Greatest Comics Magazine — and superstar writer Dan Slott is about to show you why! After being separated across dimensions, the Thing, the Human Torch, the Invisible Woman and Mister Fantastic reunite at long last! But as Reed and Sue return home with their children, they find that big things have changed! Ben Grimm and Alicia Masters have major plans of their own — like a wedding, years in the making! And a new fabulous foursome, the Fantastix, are muscling in on the scene! But now that the FF are back, can Doctor Doom be far behind? And what about Galactus? Brace yourself for the World-Eater vs. the Lord of Latveria, with the fate of the planet hanging in the balance!",
                    "https://i.annihil.us/u/prod/marvel/i/mg/8/a0/603d5b82a5bc0.jpg"
                )
                ConstantData(
                    " Superior Spider-Man Vol. 2: Otto-matic (Trade Paperback)",
                    "",
                    "https://i.annihil.us/u/prod/marvel/i/mg/c/f0/5df3fc8b3c883.jpg"
                )
                ConstantData(
                    "100th Anniversary Special (2014) #1",
                    "CELEBRATE 100 YEARS OF EARTH&#39;S MIGHTIEST HEROES &ndash; THE AVENGERS! Following the failed Badoon invasion of Earth and America&#39;s disappearance into the Negative Zone, how will the Avengers of 2061 cope?!",
                    "https://i.annihil.us/u/prod/marvel/i/mg/3/a0/53c406e09649c.jpg"
                )
                ConstantData(
                    "100th Anniversary Special (2014) #1",
                    "It&#39;s 100 years after the wall-crawler&#39;s creation, but when the Kingpin has taken Spider-Man&#39;s ultra-powerful techno-symbiote suit, Spider-Man will need to prove once again why he is the world&#39;s greatest super hero.",
                    "https://i.annihil.us/u/prod/marvel/i/mg/c/20/53b1be72eec81-jpg"
                )
                ConstantData(
                    "100th Anniversary Special (2014) #1",
                    "A REMARKABLE ARTIFACT FROM THE FUTURE OF MARVEL COMICS! It's 2061 and the world of the Fantastic Four has turned upside-down, complete with the granddaughter of Doom...and the Richards-Banner twins?",
                    "https://i.annihil.us/u/prod/marvel/i/mg/3/20/53a85058a61f0.jpg"
                )
                ConstantData(
                    " Fantastic Four by Dan Slott Vol. 1 (Hardcover)",
                    "Collects Fantastic Four (2018) #1-11, Fantastic Four Wedding Special (2018) #1. The Fantastic Four is the World’s Greatest Comics Magazine — and superstar writer Dan Slott is about to show you why! After being separated across dimensions, the Thing, the Human Torch, the Invisible Woman and Mister Fantastic reunite at long last! But as Reed and Sue return home with their children, they find that big things have changed! Ben Grimm and Alicia Masters have major plans of their own — like a wedding, years in the making! And a new fabulous foursome, the Fantastix, are muscling in on the scene! But now that the FF are back, can Doctor Doom be far behind? And what about Galactus? Brace yourself for the World-Eater vs. the Lord of Latveria, with the fate of the planet hanging in the balance!",
                    "https://i.annihil.us/u/prod/marvel/i/mg/8/a0/603d5b82a5bc0.jpg"
                )
                ConstantData(
                    " Superior Spider-Man Vol. 2: Otto-matic (Trade Paperback)",
                    "",
                    "https://i.annihil.us/u/prod/marvel/i/mg/c/f0/5df3fc8b3c883.jpg"
                )
                ConstantData(
                    "100th Anniversary Special (2014) #1",
                    "CELEBRATE 100 YEARS OF EARTH&#39;S MIGHTIEST HEROES &ndash; THE AVENGERS! Following the failed Badoon invasion of Earth and America&#39;s disappearance into the Negative Zone, how will the Avengers of 2061 cope?!",
                    "https://i.annihil.us/u/prod/marvel/i/mg/3/a0/53c406e09649c.jpg"
                )
                ConstantData(
                    "100th Anniversary Special (2014) #1",
                    "It&#39;s 100 years after the wall-crawler&#39;s creation, but when the Kingpin has taken Spider-Man&#39;s ultra-powerful techno-symbiote suit, Spider-Man will need to prove once again why he is the world&#39;s greatest super hero.",
                    "https://i.annihil.us/u/prod/marvel/i/mg/c/20/53b1be72eec81-jpg"
                )
                ConstantData(
                    "100th Anniversary Special (2014) #1",
                    "A REMARKABLE ARTIFACT FROM THE FUTURE OF MARVEL COMICS! It's 2061 and the world of the Fantastic Four has turned upside-down, complete with the granddaughter of Doom...and the Richards-Banner twins?",
                    "https://i.annihil.us/u/prod/marvel/i/mg/3/20/53a85058a61f0.jpg"
                )
            }
        }

        fun getSeries(): List<ConstantData> {
            return List(5) {
               ConstantData(
                   "She Hulk",
                   " She-Hulk: Attorney at Law, Jennifer Walters (Tatiana Maslany)—an attorney specializing in superhuman-oriented legal cases—must navigate the complicated life of a single, 30-something who also happens to be a green 6-foot-7-inch superpowered hulk.",
                   "https://cdn.marvel.com/content/1x/shehulk_lob_crd_04.jpg"
               )
                ConstantData(
                    "Wanda Vision",
                    "Marvel Studios' captivating new series \"WandaVision,\" which premieres in early 2021 on Disney+. Starring Elizabeth Olsen and Paul Bettany, \"WandaVision\" marks the first series from Marvel Studios streaming exclusively on Disney+. The series is a blend of classic television and the Marvel Cinematic Universe in which Wanda Maximoff and Vision—two super-powered beings living idealized suburban lives—begin to suspect that everything is not as it seems.",
                    "https://cdn.marvel.com/content/1x/wandavision_lob_crd_06.jpg"
                )
                ConstantData(
                    "Werewolf By Night",
                    "Marvel Studios' 'Werewolf By Night' is coming to Disney+ on October 7, 2022.",
                    "https://cdn.marvel.com/content/1x/werewolfbynight_lob_crd_01.jpg"
                )
                ConstantData(
                "Moon Knight",
                "Moon Knight follows Steven Grant, a mild-mannered gift-shop employee, who becomes plagued with blackouts and memories of another life. Steven discovers he has dissociative identity disorder and shares a body with mercenary Marc Spector. As Steven/Marc's enemies converge upon them, they must navigate their complex identities while thrust into a deadly mystery among the powerful gods of Egypt.",
                "https://cdn.marvel.com/content/1x/moonknight_lob_crd_05.jpg"
                )
                ConstantData(
                "What if",
                "What If…? flips the script on the MCU, reimagining famous events from the films in unexpected ways. Marvel Studios’ first animated series focuses on different heroes from the MCU, featuring a voice cast that includes a host of stars who reprise their roles. Directed by Bryan Andrews with AC Bradley as head writer, “What If…?\" launches exclusively on Disney+ on August 11, 2021.",
                "https://cdn.marvel.com/content/1x/whatif_lob_crd_01.jpg"
                )
            }
        }
    }
}